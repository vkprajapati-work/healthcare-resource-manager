...
**Current state:** `apps/backend` has a working, tested Express + Mongoose API covering more than `resources`: it also has `auth`, `doctors`, `drivers`, `vehicles`, `files`, and `seed` modules. `GET /resources` bridges in `doctors`/`vehicles` read-only (mapped into the resource shape, counted in `meta.counts`) — but writes still only touch the native `resources` collection (`POST /resources` is server-blocked in favor of `POST /doctors`/`POST /vehicles`), and `drivers` isn't bridged at all. Backend test coverage covers `resources`, `auth` (service unit tests + route/middleware guard tests), `health`, and shared utils — `doctors`, `drivers`, `vehicles`, `files`, and `seed` have no tests yet. See [PROJECT_CONTEXT.md](docs/PROJECT_CONTEXT.md) §8a for the full design. `apps/frontend` is functional: Vite + React 18 + strict TS + Tailwind; three layouts (public `RootLayout`, `AuthLayout`, admin `DashboardLayout`); cookie-session auth (login, session hint so anonymous visitors never call `/auth/me`, silent refresh-token retry, `ProtectedRoute` role guard); a shared component kit (`ui/` + `common/`); global mutation-error toasts. Working pages: public `/` — infinite-scroll ("load more") resource cards (images with fallback, text search + ambulance/doctor type filter in the URL, `meta.counts` header) rather than numbered pagination; admin `/admin/doctors|drivers|vehicles` — paginated list tables plus create/edit/delete dialogs (with confirmation) posting multipart to the rich APIs (doctor/driver profile image, vehicle photos, driver→vehicle assignment select, one-time provisioned-login reveal — the create response key is `login`, not `provisionedLogin`). Jest + RTL tests cover the `admin` dashboard components, `vehicles`, and shared `ui`/`common`/`lib`/`hooks` code — `resources`, `drivers`, and `auth` have no frontend tests yet. Not built yet: document upload UI (backend already accepts doctor/driver ID & license documents via multipart), numbered pagination on the public list, Husky/lint-staged.
...
```text
apps/frontend    React SPA — feature-based: src/features/<feature>/{api,components,hooks,schemas,types,index.ts} — built
