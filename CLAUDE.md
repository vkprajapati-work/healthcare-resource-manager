...
**Current state:** `apps/backend` has a working, tested Express + Mongoose API covering more than `resources`: it also has `auth`, `doctors`, `drivers`, `vehicles`, `files`, and `seed` modules. `GET /resources` bridges in `doctors`/`vehicles` read-only (mapped into the resource shape, counted in `meta.counts`) — but writes still only touch the native `resources` collection, and `drivers` isn't bridged at all. See [PROJECT_CONTEXT.md](docs/PROJECT_CONTEXT.md) §8a for the full design. `apps/frontend` is functional: Vite + React 18 + strict TS + Tailwind; three layouts (public `RootLayout`, `AuthLayout`, admin `DashboardLayout`); cookie-session auth (login, session hint so anonymous visitors never call `/auth/me`, silent refresh-token retry, `ProtectedRoute` role guard); a shared component kit (`ui/` + `common/`); global mutation-error toasts. Working pages: public `/` — paginated resource cards (images with fallback, ambulance/doctor type filter in the URL, `meta.counts` header); admin `/admin/doctors|drivers|vehicles` — paginated list tables plus create dialogs posting multipart to the rich APIs (doctor/driver profile image, vehicle photos, driver→vehicle assignment select, one-time provisioned-login reveal — the create response key is `login`, not `provisionedLogin`). Jest + RTL (14 tests). Not built yet: edit/delete UI, document uploads, search inputs.
